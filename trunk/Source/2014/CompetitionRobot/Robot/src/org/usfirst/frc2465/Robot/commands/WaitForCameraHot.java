...
public class  WaitForCameraHot extends Command {
    ...
    boolean quit_at_five_seconds;
    
    public WaitForCameraHot(boolean timeout) {
        requires(Robot.camera);
        quit_at_five_seconds = timeout;
    }
    
    public WaitForCameraHot() {
        // Use requires() here to declare subsystem dependencies
        requires(Robot.camera);
	quit_at_five_seconds = true;
    ...
    protected boolean isFinished() {
        if ( Robot.camera.isNearestTargetHot() ) {
            return true;
        }
        else if ( quit_at_five_seconds && ( DriverStation.getInstance().getMatchTime() > 5.0 ) ) {
